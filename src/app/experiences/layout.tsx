...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  const fullPathnam = queryString ? `${pathname}?${queryString}` : pathname;
  ...
  return (
    <>
      ...
      <footer className="fixed bottom-0 left-0 z-20 w-full h-16 bg-white border-t border-gray-200 shadow-sm md:items-center md:justify-between md:p-6 dark:bg-gray-800 dark:border-gray-600 justify-around">
        <ButtonGroup className="flex items-stretch h-full w-full">
          {buttons.map((btn) => (
            <Button
              variant="text"
              className={`grow ${
                fullPathnam.includes(btn.path) ? "dark:text-white" : ""
              }`}
              key={"btn-" + btn.label}
              href={btn.path}
              component={Link}
              disabled={fullPathnam.includes(btn.path)}
            >
              {btn.label}
            </Button>
          ))}
